...
template<class T> 
void TTree<T>::display_thread() {
	inorder_thread(*root);
	std::cout << std::endl;
}

template<class T> 
node<T> TTree<T>::inorder_thread(node<T> &t) {
	auto pleasehelp = t;
	
	
	std::packaged_task<node<T>(node<T>&)>
	task(std::bind(&TTree::inorder_thread, this, std::placeholders::_1));
	auto helpme = task.get_future();
	
	std::thread(std::move(task), std::ref(pleasehelp)).join();
	pleasehelp = helpme.get();
